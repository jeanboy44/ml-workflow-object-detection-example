...
### 실습 결과물 미리 확인하기 2 (streamlit)
1. streamlit app 실행
```bash
cd apps/object_detection_demo
uv run streamlit run main.py
```
1. 모델명 입력: `study.object_detection.yolo26n_pretrained`
3. 버전 선택: `3`
1. 사진 선택
1. run
---
...
#### 데이터 다운로드
```bash
uv run scripts/dbx_volumes.py download /Volumes/study/object_detection/datasets/PCB_DATASET/ --dst-path ./data/tmp
```
...
#### 데이터 분할 → YOLO 포맷 변환 
```bash
uv run split_pcb_dataset.py \
  --base-dir ../../data/PCB_DATASET \
  --output-dir ../../data/pcb_splits
```
```bash
uv run prepare_yolo_dataset.py \
