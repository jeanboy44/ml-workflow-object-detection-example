...
#### 실험
```bash
# uv run experiments/test_latest_models/detr_pre_trained_model.py "data/sample_data/dog_01.jpg" # DETR
uv run experiments/test_latest_models/yolo_pre_trained_model.py "data/sample_data/dog_02.jpg" # YOLO
# uv run experiments/test_latest_models/grounddino_zero_shot_inference.py "data/sample_data/cat_03.jpg" "cat" # GROUND DINO
```
---
...
#### 시각화 스크립트
```bash
uv run experiments/eda/eda_pcb_data.py
```
#### 노트북 파일
`experiments/eda/eda_pcb_data.ipynb`
...
#### 최신 모델로 확인
```bash
uv run experiments/test_latest_models/detr_pre_trained_model.py "data/sample_data/01_missing_hole_11.jpg" # DETR
uv run experiments/test_latest_models/yolo_pre_trained_model.py "data/sample_data/04_open_circuit_16.jpg" # YOLO
uv run experiments/test_latest_models/grounddino_zero_shot_inference.py "data/sample_data/01_missing_hole_11.jpg" "circle" # GROUND DINO
```
---
...
#### 데이터 분할 → YOLO 포맷 변환 
```bash
uv run experiments/exp01_train_yolo/split_pcb_dataset.py --base-dir data/PCB_DATASET --output-dir data/pcb_splits
uv run experiments/exp01_train_yolo/prepare_yolo_dataset.py --split-dir data/pcb_splits --output-dir data/pcb_yolo
```
...
#### Fast Run으로 파이프라인 검증
```bash
uv run experiments/exp01_train_yolo/train_yolo.py --data-yaml data/pcb_yolo/data.yaml --model artifacts/yolo/yolo26n.pt --epochs 5 --imgsz 320 --batch 2 --fraction 0.01
```
...
#### hydra를 활용한 config 관리
```bash
uv run experiments/exp01_train_yolo/train_yolo_hydra.py --config-name test_config
```

#### From Scratch
```bash
uv run experiments/exp01_train_yolo/train_yolo_hydra.py --config-name scratch_config
```
...
#### Transfer Learning
```bash
uv run experiments/exp01_train_yolo/train_yolo_hydra.py --config-name transfer_config
```

#### hyper parameter 튜닝
```bash
uv run experiments/exp01_train_yolo/train_yolo_hydra.py --config-name transfer_config train.imgsz=640 train.epochs=120 train.batch=16 train.mosaic=0.2 train.lr0=0.0025 train.warmup_epochs=5
```
---
...
#### run에서 모델 다운로드하기
```bash
uv run scripts/download_model_from_run.py 318bc2c545ee46618f236fe83de830b3 "weights/best_model" --download-dir "artifacts/runs"
```
